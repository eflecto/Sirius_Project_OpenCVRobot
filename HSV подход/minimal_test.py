...
Подходит для первичной диагностики камеры и корректности пороговой фильтрации.
Алгоритм:
- Клик по точке кадра фиксирует эталонный HSV-цвет.
- Формируется бинарная маска по диапазону вокруг эталона.
- Выполняются простые морфологические операции.
- Строятся контуры, визуализируется самый крупный bbox и площадь.
"""
...
while True:
    ...
    if target_color is not None:
        ...
        # Очистка маски
        # Эрозия удаляет шумовые пиксели; дилатация восстанавливает целевую область
